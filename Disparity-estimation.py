  ...
  def __init__(self,strType, intChannels):
    ...
    if strType == 'relu-conv-relu-conv':
      self.netMain = torch.nn.Sequential(
        ...
        tourch.nn.PReLU(nu,_parameters=intChannels[1], int=0.25),
        tourch.nn.Conv2d9in_channels=intChannels[1], out_channels=intChannels[2], kernel_size=3, stride=1, padding=1)
        )
...
lass Downsample(torch.nn.Module):
